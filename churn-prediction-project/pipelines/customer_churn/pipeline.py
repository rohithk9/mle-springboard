...
def get_pipeline(
    ...
    # Validate parameters
    if not region:
        raise ValueError("The 'region' parameter is required.")
    if not role:
        raise ValueError("The 'role' parameter is required.")
    if not default_bucket:
        raise ValueError("The 'default_bucket' parameter is required.")
    ...
    assert default_bucket is not None, "Default bucket must be provided."
    assert model_package_group_name is not None, "Model package group name must be provided."

    # Print debug info
    print("Default Bucket:", default_bucket)
    print("Model Package Group Name:", model_package_group_name)
    print("Pipeline Name:", pipeline_name)
    print("Input Data:", input_data.default_value)
    print("Batch Data:", batch_data.default_value)
    ...
    name="ChurnTransform",
    transformer=transformer,
    inputs=TransformInput(data=batch_data,content_type="text/csv", split_type="Line")
    )
    ...
    step_config_file = ProcessingStep(
        ...
        job_arguments=[
            "--modelname",step_create_model.properties.ModelName,
            "--bias-report-output-path",bias_report_output_path,
            "--clarify-instance-type",clarify_instance_type,
            "--default-bucket",default_bucket,
            "--num-baseline-samples","50",
            "--instance-count","1"],
    ...
    data_config = sagemaker.clarify.DataConfig(
        ...
        label=0,
        headers=['target', 'esent', 'eopenrate', 'eclickrate', 'avgorder',
                 'ordfreq', 'paperless', 'refill', 'doorstep',
                 'first_last_days_diff', 'created_first_days_diff',
                 'favday_Friday', 'favday_Monday', 'favday_Saturday',
                 'favday_Sunday', 'favday_Thursday', 'favday_Tuesday',
                 'favday_Wednesday', 'city_BLR', 'city_BOM',
                 'city_DEL', 'city_MAA'],
